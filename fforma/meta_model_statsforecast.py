...
def check_is_fitted(
    estimator: Any, attributes: Union[str, List[str], Tuple[str, ...]]
) -> bool:
...
# For polars an additional step is needed to convert pandas frequency to polars frequency
def get_freq_for_statsforecast(y_panel_df: pd.DataFrame) -> str:
    freq_df = y_panel_df.groupby(by="unique_id", as_index=False).agg(
        {"ds": pd.infer_freq}
    )
    if freq_df.ds.nunique() == 1:
        return freq_df.ds[0]
    else:
        raise ValueError(f"Multiple frequencies found in data: {freq_df.ds.unique()}")


def create_statsforecast_column_mapping(
    statsforecast_models: List[Any], model_names: List[str]
) -> Dict[str, str]:
    """
...
class MetaModels:
    ...
    def _fit_statsforecast(
        ...
        try:
            self.statsforecast_obj_ = StatsForecast(
                ...
                freq=get_freq_for_statsforecast(y_panel_df),
                n_jobs=1,
            )
            ...
            print(
                f"✅ Fitted {len(statsforecast_models)} StatsForecast models successfully"
            )

        except Exception as e:
            raise RuntimeError(f"Failed to fit StatsForecast models: {e}")

    def _fit_non_statsforecast(
        self, y_panel_df: pd.DataFrame, non_statsforecast_models: Dict[str, Any]
    ) -> None:
        """
        ...
        for model_name, model_template in non_statsforecast_models.items():
            ...
            for unique_id, group in y_panel_df.groupby("unique_id"):
                try:
                    ...
                    raise RuntimeError(
                        f"Warning: Failed to fit {model_name} for {unique_id}:" f" {e}"
                    )
    ...
    def fit(self, y_panel_df: pd.DataFrame) -> "MetaModels":
        ...
        for model_name, model in self.models.items():
            ...
            if hasattr(model_instance, "__module__") and "statsforecast" in str(
                model_instance.__module__
            ):
        ...
        self._fit_statsforecast(
            y_panel_df, statsforecast_models, statsforecast_model_names
        )
    ...
    def _combine_predictions(
        df_predictions: List[Optional[pd.DataFrame]],
    ) -> pd.DataFrame:
    ...
    def _predict_statsforecast(self, y_hat_df: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        if hasattr(self, "statsforecast_obj_") and self.statsforecast_obj_:
            try:
                ...
                print(
                    f"Generating predictions using fitted StatsForecast models (h={h})..."
                )
                ...
                if hasattr(self, "statsforecast_column_mapping_"):
                    forecasts = forecasts.rename(
                        columns=self.statsforecast_column_mapping_
                    )
                    print(
                        f"Applied column mapping: {self.statsforecast_column_mapping_}"
                    )
                else:
                    print(
                        "Warning: No pre-created column mapping found, using original column names"
                    )

                # Filter forecasts to match requested forecast dates
                forecasts = forecasts.merge(
                    y_hat_df, on=["unique_id", "ds"], how="inner"
                )

                if forecasts.empty:
                    raise RuntimeError(
                        f"Failed to generate predictions for {y_hat_df.shape[0]} rows"
                    )

                print(
                    f"✅ Generated predictions using fitted models for {len(self.statsforecast_model_names_)} StatsForecast models"
                )
                return forecasts

            except Exception as e:
                raise RuntimeError(
                    f"Warning: Failed to generate StatsForecast predictions from "
                    f"fitted models: {e}"
                )
        return None

    def _predict_non_statsforecast(
        ...
        if (
            hasattr(self, "non_statsforecast_models_")
            and self.non_statsforecast_models_
        ):
            for (
                model_name,
                fitted_models_dict,
            ) in self.non_statsforecast_models_.items():
                model_predictions = []

                for unique_id, group in y_hat_df.groupby("unique_id"):
                    if (
                        unique_id in fitted_models_dict
                        and fitted_models_dict[unique_id] is not None
                    ):
                        h = len(group)
                        ...
                        try:
                            ...
                            if len(preds) < h:
                                preds = np.concatenate(
                                    [
                                        preds,
                                        np.repeat(
                                            preds[-1] if len(preds) > 0 else 0.0,
                                            h - len(preds),
                                        ),
                                    ]
                                )
                            ...
                            raise RuntimeError(
                                f"Warning: Failed to predict {model_name} for {unique_id}: {e}"
                            )
                    else:
    ...
    def predict(self, y_hat_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not required_attrs:
            raise ValueError(
                "This MetaModels instance is not fitted yet. Call 'fit' with appropriate arguments before using this method."
            )
...
def temp_holdout(
    y_panel_df: pd.DataFrame, val_periods: int
    ...
    train = (
        y_panel_df.groupby("unique_id")
        .apply(lambda df: df.head(-val_periods))
...
def calc_errors(
    ...
    y_hat_panel_fun = lambda model_name: y_panel_df[
    ...
    for model_name in model_names:
        ...
        errors_mase[model_name] = evaluate_panel(
            y_panel, y_hat_panel, mase, y_insample_df, seasonality
        )
...
def get_prediction_panel(
    y_panel_df: pd.DataFrame, h: int, freq: Optional[str]
) -> pd.DataFrame:
    ...
    for idx, row in df.iterrows():
        ...
        df_ds = pd.DataFrame({"unique_id": unique_id, "ds": date_range})
